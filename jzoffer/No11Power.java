...
public class No11Power {
    ...
    public static double power(double base,int exponent){
    	if(equal(0.0,base)&&exponent<0)//底数为0，指数为负数
    		throw new RuntimeException("Wihle exponent is minus, the base cant't be zero");
    	int absExponent = exponent;
    	if(exponent<0)//底数不为零，指数为负数
    		absExponent = ~exponent+1;//负数各位求反加一，得到相反数
